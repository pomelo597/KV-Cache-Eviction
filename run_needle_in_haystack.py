...
class LLMNeedleHaystackTester:
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if(self.model_provider in ["LLaMA3", "Mistral"]):
            ...
            prompt_enc = self.enc(prompt, return_tensors="pt")
            input_ids = prompt_enc['input_ids'].to(self.model_to_test.device)

            decoded_no_special = self.enc.decode(input_ids[0], skip_special_tokens=True)

            no_spec_ids = self.enc(decoded_no_special, add_special_tokens=False, return_tensors="pt")["input_ids"][0].tolist()

            head_size = getattr(global_vars, "head_size", 0)
            tail_size = getattr(global_vars, "tail_size", 0)

            spans, total_tokens = self._build_segment_boundaries_head_tail_sentence(
                tokenizer=self.enc,
                input_ids_no_special=no_spec_ids,
                head_size=head_size,
                tail_size=tail_size,
                merge_small=False,   
                min_len=16,          
            ...
            sample_id = getattr(global_vars, "current_sample_id", 0)  
            global_vars.global_segment_boundaries[sample_id] = spans
    ...
    def _build_segment_boundaries_head_tail_sentence(
        ...
        decoded = tokenizer.decode(input_ids_no_special, skip_special_tokens=False)
        sents = self._sent_split(decoded)
        ...
        for sent in sents:
            ...
            if n == 0:
                continue
            token_spans.append([cur, cur + n])  # [start, end)
            cur += n

        total_tokens = cur

        head = max(0, min(head_size, total_tokens))
        tail = max(0, min(tail_size, total_tokens - head))
        mid_start = head
        ...
        if tail > 0:
            spans.append([total_tokens - tail, total_tokens])

        if merge_small and spans:
            merged = []
            cur_s, cur_e = spans[0]
            for s, e in spans[1:]:
                if (cur_e - cur_s) < min_len or (s - cur_e) <= join_gap:
                    cur_e = e
                else:
